...
fun SignupScreen(navController: NavController, viewModel: SignupViewModel = hiltViewModel()) {
    ...
    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.BottomCenter) {
        Column(Modifier.fillMaxSize()) {
            ...
            Row {
                Checkbox(checked = viewModel.state.isPrivacyAccepted, onCheckedChange = {
                    viewModel.state = viewModel.state.copy(isPrivacyAccepted = it)
                })
                Text(
        ...
        Text(text = "Есть аккаунт? Войти", Modifier.clickable {
            navController.navigate(NavDestinations.Login) {
                popUpTo(NavDestinations.Signup) {
                    inclusive = true
                }
            }
        })
